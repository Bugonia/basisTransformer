...
coefficients of another module's writes. We introduce a basis/coefficient view
...
modulation in residual networks. Second, we derive the Attention and FFN
...
substantial part of the standard Transformer block advantage, beyond ordering
and optimization effects. Fourth, we instantiate the framework as a diagnostic
path for pretrained open models through basis inventory, logit attribution, and
...
so the paper does not claim a complete scaling law for large language models.
Third, the term ``basis'' means an overcomplete learned write dictionary, not a
linearly independent basis. Fourth, the framework does not imply that
individual write directions are monosemantic. Finally, pretrained open-model
...
We introduced a basis/coefficient view of residual updates and used it to
distinguish direct write access from coefficient-only modulation. In
Transformers, controlled topology experiments show that preserving both
Attention and FFN direct write families is substantially stronger than allowing
one module only to modulate the other's coefficients. Direct residual write
access is therefore not an implementation detail of Transformer blocks; it is a
general architectural resource that determines which learned direction families
can directly shape a model's computation.
