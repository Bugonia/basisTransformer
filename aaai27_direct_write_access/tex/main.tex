...
complementary output-projection absorption control shows that an Attention
output projection can be functionally redundant when it is not a direct
residual write outlet, sharpening the distinction between parameterization and
write access. These results support the view that direct write access is an
architectural resource in Transformers and reframe residual streams as write
economies: shared spaces where model quality depends on which heterogeneous
basis families can directly write and how their dynamic coefficients are
coupled.
\end{abstract}
