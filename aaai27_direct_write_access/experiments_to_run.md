...
- complete: 7 variants x 5 seeds;
- all variants use 25.6M matched parameters;
- detailed paired statistics are generated by
  `aaai27_direct_write_access/scripts/analyze_topology_results.py`;
- use this five-seed table as the primary controlled ablation in the paper, but
  phrase the claim as evidence consistent with direct-write access rather than
  as a complete causal isolation.
...
### E0: Rank-Controlled Direct-Write Intervention

Priority:

- highest; this is the cleanest answer to the critique that direct write access
  was not isolated.
...
> If `rank_write` improves over its block baseline more than the matched
> `rank_coeff` control, then direct residual write access has cleaner causal
> support. If the matched controls are indistinguishable, the paper should
> retreat to a weaker topology-ablation claim.
