...
BASE_RUN="${BASE_RUN:-enwik8_loop_transformer_${VARIANT}_${NORM}_${NORM_KIND}_${OPTIMIZER}_${N_LAYER}l_${N_EMBD}d_ctx${BLOCK_SIZE}_bs${BATCH_SIZE}_test005_100k_earlystop10_lrdecay30k}"

UNIQUE_LAYERS_STRING="${UNIQUE_LAYERS:-1 2 4 8}"
SEEDS_STRING="${SEEDS:-1 2}"
read -r -a UNIQUE_LAYER_ARRAY <<< "$UNIQUE_LAYERS_STRING"
...
for seed in "${SEED_ARRAY[@]}"; do
  for n_unique in "${UNIQUE_LAYER_ARRAY[@]}"; do
    if (( n_unique < 1 || n_unique > N_LAYER )); then
      echo "Skipping n_unique_layers=${n_unique}: must be in [1, ${N_LAYER}]." >&2
      continue
    fi

    gpu="${GPU_ARRAY[$slot]}"
    run_name="${BASE_RUN}_seed${seed}_u${n_unique}"
...
"$PYTHON_BIN" experiments/loop_transformer/summarize_loop_transformer.py \
  "runs/block_residuals/${BASE_RUN}_seed*/summary.csv" \
  --baseline-unique-layers "$N_LAYER" \
