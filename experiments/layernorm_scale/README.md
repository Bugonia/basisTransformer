...
token      每个 token id、hidden channel、block 内 Norm site 一个可学习 gamma
```

LayerNorm 的 bias 保持现有 `--bias` 设置，默认仍可学习。这样实验主要隔离 gamma 的
参数化差异，而不是同时移除 affine bias。

`token` 模式里，词表大小为 `v`、hidden width 为 `d`、Transformer block 层数为
`l` 时，每个 block 的 `ln1` 和 `ln2` 各持有一个 `[v, d]` gamma 表，二者不共享。
默认 pre-LayerNorm 标准 Transformer 因此存储 `2*v*d*l` 个 token-channel gamma；
最终 `ln_f` 使用固定 gamma=1，不会额外引入第 `l+1` 组 token gamma。
...
- `scalar` 每个 Norm 只保留 1 个 gamma 参数，可以测试“需要一个整体幅度旋钮”还是
  “需要逐通道重标定”。
- `token` 每个 Transformer block 的每个 active Norm site 保留
  `vocab_size * n_embd` 个 gamma 参数，同一 token id 在该层该 Norm site 的所有
  位置使用同一条 per-channel 缩放向量。
- 默认 pre-LayerNorm 标准 Transformer 共有每层 `ln1`、`ln2` 加最终 `ln_f`；8 层时
  共 17 个 Norm 层。`learned`/`fixed_one`/`scalar` 按这些 Norm 层计数；`token`
  则按 block 内 active Norm site 计数，默认保持 `2*v*d*l`。
