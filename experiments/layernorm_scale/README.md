...
token      每个 token id、每个 Transformer block 一个可学习 gamma 标量
```

LayerNorm 的 bias 保持现有 `--bias` 设置，默认仍可学习。这样实验主要隔离 gamma 的
参数化差异，而不是同时移除 affine bias。

`token` 模式按你说的 `v*l` 存储 gamma：词表大小为 `v`、Transformer block 层数为
`l` 时，每个 block 持有一个长度为 `v` 的 gamma 向量。这个向量在该 block 内的
`ln1/ln2` 之间共享；最终 `ln_f` 使用固定 gamma=1，因此不会额外引入第 `l+1` 组
token gamma。
