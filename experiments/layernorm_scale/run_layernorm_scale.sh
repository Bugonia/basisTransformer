...
BASE_RUN="${BASE_RUN:-enwik8_layernorm_scale_standard_pre_layernorm_muon_8l_512d_ctx512_bs256_lr2e3_test005_100k_earlystop10_lrdecay30k}"
...
NORM_SCALES_STRING="${NORM_SCALES:-learned fixed_one scalar token}"
SEEDS_STRING="${SEEDS:-1 2}"
read -r -a NORM_SCALE_ARRAY <<< "$NORM_SCALES_STRING"
...
for seed in "${SEED_ARRAY[@]}"; do
  for norm_scale in "${NORM_SCALE_ARRAY[@]}"; do
    gpu="${GPU_ARRAY[$slot]}"
    run_name="${BASE_RUN}_seed${seed}_${norm_scale}"
...
"$PYTHON_BIN" experiments/layernorm_scale/summarize_layernorm_scale.py \
