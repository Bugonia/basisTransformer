...
read -r -a NORM_ARRAY <<< "$NORMS_STRING"
...
for seed in "${SEED_ARRAY[@]}"; do
  for norm in "${NORM_ARRAY[@]}"; do
    gpu="${GPU_ARRAY[$slot]}"
    run_name="${BASE_RUN}_seed${seed}_${norm}"
...
"$PYTHON_BIN" experiments/rmsnorm_placement/summarize_rmsnorm_placement.py \
