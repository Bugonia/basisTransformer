#!/usr/bin/env python3
"""Summarize standard Transformer token-mixer ablations."""
...
DEFAULT_BASE_RUN = (
    "enwik8_sequence_mixers_fla_muon_8l_512d_ctx512_bs256_lr2e3_"
...
SEQUENCE_MIXER_BY_VARIANT = {
    "standard": "softmax_attention",
    "standard_linear_attn": "fla_linear_attention",
...
def infer_sequence_mixer(variant: str, row: Dict[str, str]) -> str:
    if row.get("sequence_mixer"):
        return row["sequence_mixer"]
    return SEQUENCE_MIXER_BY_VARIANT.get(variant, "unknown")


def infer_ffn_kind(row: Dict[str, str]) -> str:
    return row.get("ffn_kind") or "gelu"


def infer_attention_gate(row: Dict[str, str]) -> str:
    return row.get("attention_gate") or "none"
        ...
        for filename in sorted(glob.glob(pattern)):
            ...
            with summary_path.open(newline="", encoding="utf-8") as f:
                for row in csv.DictReader(f):
                    ...
                    row["sequence_mixer"] = infer_sequence_mixer(variant, row)
                    row["ffn_kind"] = infer_ffn_kind(row)
                    row["attention_gate"] = infer_attention_gate(row)
    ...
    for row in aggregate_rows:
        aggregate_table.append(
            [
                row["variant"],
                row["n"],
                fmt_number(safe_float(row["parameters_mean"]), digits=7),
                fmt_mean_std(
